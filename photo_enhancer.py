...
class RRDBNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, num_features=64, num_blocks=23, num_grow_ch=32, scale=4):
        super(RRDBNet, self).__init__()
        self.scale = scale
        
        self.conv_first = nn.Conv2d(in_channels, num_features, 3, 1, 1)
        self.body = nn.ModuleList([RRDB(num_features, num_grow_ch) for _ in range(num_blocks)])
...
class NaturalPhotoEnhancer:
    ...
    def load_model(self, model_type='realesrgan'):
        ...
        if self.model_path and os.path.exists(self.model_path):
            try:
                self.model = RRDBNet(in_channels=3, out_channels=3, num_features=64, num_blocks=23, num_grow_ch=32, scale=4)
